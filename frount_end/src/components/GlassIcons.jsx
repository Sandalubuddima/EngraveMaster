...
const GlassIcons = ({ items, className }) => {
  ...
  const handleClick = (index) => {
    ...
    setTimeout(() => setActiveIndex(null), 300); // remove animation after 300ms
  };
  ...
  return (
    <div
      className={`grid gap-[10em] grid-cols-2 md:grid-cols-3 mx-auto py-[3em] overflow-visible ${className || ""}`}
    >
      {items.map((item, index) => (
        <button
          ...
          className={`group relative bg-transparent outline-none w-[10em] h-[10em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] transition-transform duration-300 ${item.customClass || ""}`}
        >
          {/* Back layer */}
          <span
